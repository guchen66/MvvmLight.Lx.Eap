...
namespace MvvmLight.Lx.Core.Extensions
{
    ...
    /// 服务定位器
    /// </summary>
    public static class ViewModelServiceLocator
    {
        public static IUnityContainer DefaultContainer => CreateContainer();
        ...
        public static void Usage<T>()
        {
            Type type = typeof(T);
            var con1 = type.GetConstructor(new[] { typeof(IUnityContainer) });
            if (con1 != null)
            {
                var instance = (T)con1.Invoke(new object[] { _container });
                
            }
            var constructors = type.GetConstructors();
            object instance2 = Activator.CreateInstance(type, _container);
        }
        ...
        private static IUnityContainerExtensionConfigurator _extensionConfigurator;

        private static UnityContainerExtension _unityContainerExtension;


        //  private static UnityContainerExtensions _unityContainerExtensions;
        ...
        public static object Get(Type typeOfInstance)
        {
            ...
            lock (_operationLock)
            {
                ...
                {
                    throw new Exception(typeOfInstance.FullName);
                }
            }
